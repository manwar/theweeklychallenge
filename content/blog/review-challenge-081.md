...
As so kindly annotated, the expression is anchored to the beginning of the string and captures a group of characters that may be repeated immediately 0 or more times. This will always start by matching the entirety of the $A string section. Then our constructed separator will always match. After this it tries to match the capture group again from the end of the separator to the end of the string. If it fails to match here it backtracks to try a capture group one shorter and start over. If it succeeds, on the other hand, the next part of the expression is evaluated, the code block, which always matches. Here she prints the capture group to output. The evaluation continues to the next and final section, the verb, and the match by definition fails.
